...
balsAppRequest.onreadystatechange = function(){
	...
	if(balsAppRequest.readyState ==4){
		...
		if(balsAppRequest.status ==200){
			...
			//Create variable and grab array - Marcus Palmer
			...
			// Add some basic class to the div container which the output will be transmitted - Marcus Palmer
			divElement.classList.add("uk-text-left"); 
			divElement.classList.add("uk-card");
			divElement.classList.add("uk-card-default");
			divElement.classList.add("uk-card-body");
			
			// Create a basic button element - Marcus Palmer
			var button = document.createElement('button');
			button.classList.add("uk-button");
			button.classList.add("uk-button-fxprodesign");
			...
			function element(item, index){
				...
				// Line break
				text += "<li><div><h2>" + index +".</h2>" 
						+ "<strong>Postcode</strong>: " + item.fields.postcode + "<br> <strong>Local Authority</strong>: " +item.fields.local_authority 
						...
						"</div><br><button>More Info</button> <button>Map</button>"
						+"</li>";
...
balsAppRequest.open('GET','http://opendata.bristol.gov.uk/api/records/1.0/search/?dataset=postcode-lookup-for-the-west-of-england-february-2018-copy&q=&rows=10&facet=status&facet=local_authority', true)
